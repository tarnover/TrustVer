...
fn init_repo_with_commits() -> TempDir {
    ...
    process::Command::new("git").args(["commit", "-m", "feat: initial [h]\n\nAuthorship: h"]).current_dir(p).output().unwrap();
    process::Command::new("git").args(["tag", "v0.1.0"]).current_dir(p).output().unwrap();

    std::fs::write(p.join("b.txt"), "world").unwrap();
    process::Command::new("git").args(["add", "."]).current_dir(p).output().unwrap();
    process::Command::new("git").args(["commit", "-m", "feat: second [hrai]\n\nAuthorship: hrai\nReviewer: t@t.com"]).current_dir(p).output().unwrap();
