...
pub fn run(range: Option<String>, json: bool) -> Result<()> {
    ...
    let git_commits = git_log_range(&repo_path, &from, &to)
        .context("failed to read git log")?;

    if git_commits.is_empty() {
        if json {
            println!("{}", serde_json::json!({"commits": 0, "message": "no commits in range"}));
        } else {
    ...
    if json {
        ...
        println!("  Commits: {} ({} untagged)", result.summary.total_commits, result.summary.untagged_commits);
